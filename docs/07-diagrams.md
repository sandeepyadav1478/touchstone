...
| **Every store, and who writes to it** | Checkpointer, span store, results files, both suite manifests — and ⚠️ the two are not symmetric: `suite/benchmark/` has no writer at all after generation, `suite/regression/` has exactly one (`touchstone suite gauntlet`). Two writers to one store is a design question and it must be visible |
